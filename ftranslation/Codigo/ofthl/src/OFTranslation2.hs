...
rmvQ (Neg f)  xs = ((Neg f'), xs++ys)
    where a = rmvQ f xs
          f' = fst a
          ys = snd a
rmvQ (f :& g) xs = (f' :& g', xs++ys++zs)
    where a = rmvQ f []
          f' = fst a
          ys = snd a
          b = rmvQ g []
...
addQuant [] f     = f
addQuant (x:xs) f = (addQuant xs (c i s f))
    where c = (m1 x)
	  i = (m2 x)
          s = (m3 x)
...
--moveQuantOut f = addQuant (saveQuant f []) (removeQuant f)
...
pnfFuncTrans = moveCuantOut . functionalTranslation
