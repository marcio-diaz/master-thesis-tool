...
#                         '-Sorts=%i'     % self.__sortsParam,
        ...
        oftBinPath = "/home/marcio/.cabal/bin/ofthl-spass"
        myTestFile = batchDir + "/" + testFile
        systemOrDie(oftBinPath, [myTestFile, myTestFile + ".spass"])
        spassCmdLine[-1] =  spassCmdLine[-1] + ".spass"
        systemOrDie("rm", [myTestFile])
        self.runWrapper(spassCmdLine, testFile+".spass", allowedErrorCodes=segFaultCode)
